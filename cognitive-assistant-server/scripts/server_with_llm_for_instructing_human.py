...
from .model_interface.tutorial_follower_human import TutorialFollowerHuman
...
human_querier = HumanQuerier(data_collector)

tutorial_follower = TutorialFollowerHuman(data_collector, human_querier)
...
class AudioTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if tutorial_follower.get_answer() is not None:
            response = tutorial_follower.get_answer()
            datachannels[LLM_OUTPUT_DC_LABEL].send(response)
            tutorial_follower.clear_answer()
            human_querier.clear_answer()
